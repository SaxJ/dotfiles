(defun saxon/ssh-push-file ()
  (interactive)
  (let* ((project-root (project-root (project-current t)))
         (file-path (buffer-file-name))
         (relative-path (file-relative-name file-path project-root))
         (default-directory project-root)
         (formatted-command (format "scp \"%s\" ubuntu@minikube:/home/ubuntu/$(basename %s)/%s" relative-path project-root relative-path)))
    (shell-command formatted-command)))

(defun saxon/ssh-pull-file ()
  (interactive)
  (let* ((project-root (project-root (project-current t)))
         (file-path (buffer-file-name))
         (relative-path (file-relative-name file-path project-root))
         (default-directory project-root)
         (formatted-command (format "scp ubuntu@minikube:/home/ubuntu/$(basename %s)/%s \"%s\"" project-root relative-path relative-path)))
    (shell-command formatted-command)))
...
(use-package general
  ...
  (general-nmap
    ...
    ;; remote
    "ru" 'saxon/ssh-push-file
    "rd" 'saxon/ssh-pull-file
